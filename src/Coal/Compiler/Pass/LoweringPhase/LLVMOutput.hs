...
generateLLOutput :: (MonadIO m, MonadMask m) => Maybe ProgressBar -> CompilerConfig -> [BuildEnvelope (Name, [IRConstruct [IRLine]])] -> CompilerT Metadata m (Either CompilerFailureMode [(Name, ByteString)])
...
irOutput :: (MonadIO m) => Maybe ProgressBar -> CompilerConfig -> FilePath -> BuildEnvelope (Name, [IRConstruct [IRLine]]) -> CompilerT Metadata m (Either SomeException (Name, ByteString))
