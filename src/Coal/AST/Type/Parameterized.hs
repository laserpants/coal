...
instantiateVars :: (MonadState s m, Supply s) => [(Name, TypeIndex Kind)] -> Environment Kind -> Type Parameter k -> m (Either KindError IndexedType)
...
instantiateTypeApplication :: (MonadState s m, Supply s) => Type Parameter k -> NonEmpty (Type Parameter k) -> ExceptT KindError (ReaderT (Environment (TypeIndex Kind), Environment Kind) m) IndexedType
...
instantiateTypeVars :: (MonadState s m, Supply s) => Type Parameter k -> ExceptT KindError (ReaderT (Environment (TypeIndex Kind), Environment Kind) m) IndexedType
instantiateTypeVars =
  \case
...
instantiateRowVars :: (MonadState s m, Supply s) => Row Parameter k (Type Parameter k) -> ExceptT KindError (ReaderT (Environment (TypeIndex Kind), Environment Kind) m) (Row TypeIndex Kind IndexedType)
instantiateRowVars =
  \case
