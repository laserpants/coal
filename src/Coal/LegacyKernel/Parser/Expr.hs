...
module Coal.LegacyKernel.Parser.Expr (expr, label) where

import Coal.Common.Label (Label (..))
import Coal.LegacyKernel.Language.Expr (Binding (..), Clause (..), Expr, Focus (..))
import qualified Coal.LegacyKernel.Language.Expr.Syntax as Syntax
import Coal.LegacyKernel.Language.Type (Type (..))
import Coal.LegacyKernel.Parser (Parser, backtickString, lexeme, some, try, ($>), (<|>))
import Coal.LegacyKernel.Parser.Identifier (constructor, name)
import Coal.LegacyKernel.Parser.Op (op)
import Coal.LegacyKernel.Parser.Prim (prim)
import Coal.LegacyKernel.Parser.Symbol
