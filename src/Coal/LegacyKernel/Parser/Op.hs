...
module Coal.LegacyKernel.Parser.Op (op) where

import Coal.LegacyKernel.Language.Op (Op (..))
import Coal.LegacyKernel.Language.Type (Type (..))
import Coal.LegacyKernel.Parser (Parser, lexeme, optional, try, ($>), (<|>))
import Coal.LegacyKernel.Parser.Symbol (brackets, pair, parens, symbol)
