...
module Coal.LegacyKernel.LLVM.IREval.Expr (IREval (..)) where

import Coal.Common.Label (Label (..))
import Coal.LegacyKernel.LLVM.IREncodable (irEncode)
import Coal.LegacyKernel.LLVM.IREval (IREval (..), IRTailContext (..))
import Coal.LegacyKernel.LLVM.IREval.Closure (irApplyClosure, irPackClosure)
import Coal.LegacyKernel.LLVM.IREval.Comment (irComment, irCommentBlock)
import Coal.LegacyKernel.LLVM.IREval.Conceal (irConceal, irConcealArgs, irReveal)
import Coal.LegacyKernel.LLVM.IREval.Expr.Match (irEvalMatch)
import Coal.LegacyKernel.LLVM.IREval.Expr.Op (irEvalOp)
import Coal.LegacyKernel.LLVM.IREval.Expr.Var (irEvalVar)
import Coal.LegacyKernel.LLVM.IREval.Malloc (irMalloc)
import Coal.LegacyKernel.LLVM.IRInstruction (ICmpCond (..), IRConstructor (..), IRInstr, TailMarker (..))
import Coal.LegacyKernel.LLVM.IRInstruction.Builders
import Coal.LegacyKernel.LLVM.IRType (IRType (..), IRTyped (..))
import Coal.LegacyKernel.LLVM.IRType.Syntax (i1, i32, i8Ptr, stringLiteral, struct)
import Coal.LegacyKernel.LLVM.IRValue (IRValue (..), irPrimValue)
import qualified Coal.LegacyKernel.Language as Syntax
import Coal.LegacyKernel.Language.Type.Arrow (arity, returnTypeOf)
