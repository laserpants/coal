...
Next, an *algebra* for a functor 𝐹 is a type 𝐴 (the *carrier*) together with a function 𝐹(𝐴) → 𝐴. This function is what the pattern matching clauses (in combination with the `@`-pattern syntax) in `fold` expressions describe:
...
The *initial* algebra (𝜇𝐹, **in**) is defined by the map **in** : 𝐹(𝜇𝐹) → 𝜇𝐹. The carrier of this algebra is the data type itself (i.e., `List<t>` in this example). The fact that this is an initial object means that there is a unique function ℎ : 𝜇𝐹 → 𝐴 that makes the diagram commute. This is the `fold` construct.
