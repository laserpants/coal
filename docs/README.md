...
An algebraic data type can be seen as the initial algebra of a functor: it provides the smallest, well-founded solution that can be consumed by a fold (a catamorphism).
Dually, a codata type corresponds to the final coalgebra of a functor: it is the largest, potentially infinite solution that can be observed or generated by an unfold (an anamorphism).
In this description, algebras and coalgebras are mirror images: by simply reversing the direction of the arrows in their definitions, one turns an algebra into a coalgebra and vice versa.
