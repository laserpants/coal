...
Next, an *algebra* for a functor 𝐹 is a type 𝐴 (the *carrier*) together with a function 𝐹(𝐴) → 𝐴. This function, called the *structure map*, is what the pattern matching clauses (in combination with the `@`-pattern syntax) in `fold` expressions describe:
