...
module Noll.Kernel.Compiler.Utils (
  testRunner,
  testModules,
  runTest,
  unsafeParseExpr,
...
import Noll.Kernel.Compiler
import Noll.Kernel.LLVM.IRConstruct (IRConstruct (..))
import Noll.Kernel.LLVM.IREncodable (IREncodable (..))
import Noll.Kernel.LLVM.IRInterpreter.Monad (IRLine)
import Noll.Kernel.Language.Module (Module (..))
import Noll.Kernel.Parser (spaces)
import Noll.Kernel.Parser.Expr (expr)
import Noll.Kernel.Parser.Module (module_)
